...
interface ServiceDetailPageProps {
  /** The service object containing all data to display. */
  service: Service;
...
/**
 * An accessible accordion item component for the FAQ section.
 * @param {object} props - Component props.
 * @param {FaqItem} props.item - The FAQ item data (question and answer).
...
/**
 * A reusable section component with a sticky title and icon on larger screens.
 * @param {object} props - Component props.
 * @param {string} props.title - The title of the section.
 * @param {React.ReactNode} props.children - The content of the section.
 * @param {JSX.Element} props.icon - The icon to display next to the title.
 */
...
/**
 * The main component for displaying the details of a specific service.
 * It lays out all the information from a Service object into a structured page.
 * @param {ServiceDetailPageProps} props - The props for the component.
 */
const ServiceDetailPage: React.FC<ServiceDetailPageProps> = ({ service, navigate, onGetAssessmentClick }) => {
  // State to manage which FAQ item is currently expanded.
  const [openFaqIndex, setOpenFaqIndex] = useState<number | null>(0);
  // Hook to animate the entire page content on load.
  const mainRef = useScrollAnimation({ threshold: 0.05 });

  // Effect to scroll to the top of the page when the service changes.
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [service.id]);
  
  // Icon mapping for the different sections of the page.
  const icons = {
